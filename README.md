...
class Program
{
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("Wraparound interval   : {0}", s.WraparoundInterval(generator.TimeSource));

        // And finally: let's ask the id-structure when this wraparound will happen
        // (we'll have to tell it the generator's epoch)
        Console.WriteLine("Wraparound date       : {0}", s.WraparoundDate(generator.Epoch, generator.TimeSource).ToString("O"));
    }
}
```
