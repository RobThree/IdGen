...
namespace IdGenTests
{
    ...
    public class ConfigTests
    {
        ...
        {
            var target = AppConfigFactory.GetFromConfig("xxx");
        }
        ...
        {
            var target = AppConfigFactory.GetFromConfig("e1");
        }
        ...
        {
            var target = AppConfigFactory.GetFromConfig("e2");
        }
        ...
        {
            Assert.AreEqual(new DateTime(2016, 1, 2, 12, 34, 56, DateTimeKind.Utc), AppConfigFactory.GetFromConfig("foo").Epoch);
            Assert.AreEqual(new DateTime(2016, 2, 1, 1, 23, 45, DateTimeKind.Utc), AppConfigFactory.GetFromConfig("bar").Epoch);
            Assert.AreEqual(new DateTime(2016, 2, 29, 0, 0, 0, DateTimeKind.Utc), AppConfigFactory.GetFromConfig("baz").Epoch);
        }

        [TestMethod]
        public void IdGenerator_GetFromConfig_ParsesTickDurationCorrectly()
        {
            Assert.AreEqual(TimeSpan.FromMilliseconds(50), AppConfigFactory.GetFromConfig("foo").TimeSource.TickDuration);
            Assert.AreEqual(new TimeSpan(1, 2, 3), AppConfigFactory.GetFromConfig("bar").TimeSource.TickDuration);
            Assert.AreEqual(TimeSpan.FromTicks(7), AppConfigFactory.GetFromConfig("baz").TimeSource.TickDuration);

            // Make sure the default tickduration is 1 ms
            Assert.AreEqual(TimeSpan.FromMilliseconds(1), AppConfigFactory.GetFromConfig("nt").TimeSource.TickDuration);
        }
